EXTRACTED_DRIVERS_6_2_0W4= \
	src/6.2.0w4/postflight.original \
	src/6.2.0w4/preflight.original \
	src/6.2.0w4/WacomTablet.prefpane.original

PATCHED_DRIVERS_6_2_0W4= \
	src/6.2.0w4/postflight.patched \
	src/6.2.0w4/preflight.patched \
	src/6.2.0w4/WacomTablet.prefpane.patched

EXTRACTED_DRIVERS+= $(EXTRACTED_DRIVERS_6_2_0W4)

PATCHED_DRIVERS+= $(PATCHED_DRIVERS_6_2_0W4)

SIGN_ME_6_2_0W4= \
	package/content.pkg/Payload/Library/Application\ Support/Tablet/WacomTabletDriver.app/Contents/Resources/TabletDriver.app \
	package/content.pkg/Payload/Library/Application\ Support/Tablet/WacomTabletDriver.app/Contents/Resources/WacomTouchDriver.app \
...
UNSIGNED_INSTALLERS+= Install\ Wacom\ Tablet-6.2.0w4-patched-unsigned.pkg
SIGNED_INSTALLERS+= Install\ Wacom\ Tablet-6.2.0w4-patched.pkg
	
# Create the installer package by modifying Wacom's original:

Install\ Wacom\ Tablet-6.2.0w4-patched-unsigned.pkg : src/6.2.0w4/Install\ Wacom\ Tablet.pkg src/6.2.0w4/Welcome.rtf src/6.2.0w4/PackageInfo src/6.2.0w4/Distribution $(PATCHED_DRIVERS_6_2_0W4) src/6.3.7.1/Wacom\ Tablet.kext src/6.3.4-3/Wacom\ Tablet\ Utility.app tools/fix_LC_VERSION_MIN_MACOSX/fixSDKVersion
	...
	cp -a -L src/6.2.0w4/Install\ Wacom\ Tablet.pkg/Contents/Resources package/
	...
	# Install patched postinstall script: Use new language manifest loader code from 5.3.7-6, new agent loader, avoid /tmp strategy for WacomMultiTouch.framework
	cp src/6.2.0w4/postflight.patched package/content.pkg/Scripts/postflight
	# New agent unloader
	cp src/6.2.0w4/preflight.patched  package/content.pkg/Scripts/preflight
	cp src/6.2.0w4/{unloadagent,loadagent} package/content.pkg/Scripts/

	# Add metadata files that weren't present in the old package style
	cp src/6.2.0w4/PackageInfo package/content.pkg
	cp src/6.2.0w4/Distribution package/

	# Add Welcome screen
	find package/Resources -type d -depth 1 -exec cp src/6.2.0w4/Welcome.rtf {}/ \;

	# Unpack payload
	cd package/content.pkg/Payload && tar --no-same-owner -xf ../../../src/6.2.0w4/Install\ Wacom\ Tablet.pkg/Contents/Archive.pax.gz
	...
	cp src/6.2.0w4/WacomTablet.prefpane.patched package/content.pkg/Payload/Library/PreferencePanes/WacomTablet.prefPane/Contents/MacOS/WacomTablet
	...
	tools/fix_LC_VERSION_MIN_MACOSX/fixSDKVersion $(FIX_SDK_6_2_0W4)

ifdef CODE_SIGNING_IDENTITY
	# Resign drivers and enable Hardened Runtime to meet notarization requirements
	codesign -s "$(CODE_SIGNING_IDENTITY)" -f --options=runtime --timestamp $(SIGN_ME_6_2_0W4)
else
	codesign --remove-signature $(SIGN_ME_6_2_0W4)
endif
...
Install\ Wacom\ Tablet-6.2.0w4-patched.pkg : Install\ Wacom\ Tablet-6.2.0w4-patched-unsigned.pkg
	productsign --sign "$(PACKAGE_SIGNING_IDENTITY)" Install\ Wacom\ Tablet-6.2.0w4-patched-unsigned.pkg Install\ Wacom\ Tablet-6.2.0w4-patched.pkg
endif

# Download, mount and unpack original Wacom installers:

src/6.2.0w4/WacomTablet_6.2.0w4.dmg :
	curl -o $@ "https://cdn.wacom.com/U/Drivers/Mac/pro/WacomTablet_6.2.0w4.dmg"
	[ $$(md5 $@ | awk '{ print $$4 }') = "76c7062eee387b4b1389366fd94cdfda" ] || (rm $@; false) # Verify download is undamaged

src/6.2.0w4/Install\ Wacom\ Tablet.pkg : src/6.2.0w4/WacomTablet_6.2.0w4.dmg
	hdiutil attach -quiet -nobrowse -mountpoint src/6.2.0w4/dmg "$<"
	rm -rf "$@"
	cp -a "src/6.2.0w4/dmg/Install Wacom Tablet.pkg" "$@"
	# The permissions on the package files are super awkward, make those more permissive for us:
	find "src/6.2.0w4/Install Wacom Tablet.pkg" -type d -exec chmod 0755 {} \;
	find "src/6.2.0w4/Install Wacom Tablet.pkg" -type f -exec chmod u+rw {} \;
	# Also copy the directories from outside the package because we need them for getting licence files
	cp -R src/6.2.0w4/dmg/{ChineseS,ChineseT,Dutch,English,French,German,Italian,Japanese,Korean,Polish,Portuguese,Russian,Spanish} src/6.2.0w4/
	hdiutil detach -force src/6.2.0w4/dmg
	touch "$@"

# Extract original files from the Wacom installers as needed:

$(EXTRACTED_DRIVERS_6_2_0W4) : src/6.2.0w4/Install\ Wacom\ Tablet.pkg
	rm -rf src/6.2.0w4/Install\ Wacom\ Tablet.pkg/Contents/Archive
	mkdir -p src/6.2.0w4/Install\ Wacom\ Tablet.pkg/Contents/Archive
	cd src/6.2.0w4/Install\ Wacom\ Tablet.pkg/Contents/Archive && tar --no-same-owner -xf ../Archive.pax.gz
	cp src/6.2.0w4/Install\ Wacom\ Tablet.pkg/Contents/Resources/postflight src/6.2.0w4/postflight.original
	cp src/6.2.0w4/Install\ Wacom\ Tablet.pkg/Contents/Resources/preflight  src/6.2.0w4/preflight.original
	cp src/6.2.0w4/Install\ Wacom\ Tablet.pkg/Contents/Archive/Library/PreferencePanes/WacomTablet.prefpane/Contents/MacOS/WacomTablet src/6.2.0w4/WacomTablet.prefpane.original

# Utility commands:

notarize-intuos2: Install\ Wacom\ Tablet-6.2.0w4-patched.pkg
	xcrun altool \
		 --notarize-app \
		 ...
		 --file "$<"
	cp "$<" "Install Wacom Tablet-6.2.0w4-patched-notarized.pkg"

staple-intuos2:
	xcrun stapler staple "Install Wacom Tablet-6.2.0w4-patched.pkg"
	cp "Install Wacom Tablet-6.2.0w4-patched.pkg" "Install Wacom Tablet-6.2.0w4-patched-stapled.pkg"

unpack-intuos2 : src/6.2.0w4/Install\ Wacom\ Tablet.pkg
	mkdir -p src/6.2.0w4/Install\ Wacom\ Tablet.pkg/Contents/Archive
	cd src/6.2.0w4/Install\ Wacom\ Tablet.pkg/Contents/Archive && tar --no-same-owner -xf ../Archive.pax.gz
